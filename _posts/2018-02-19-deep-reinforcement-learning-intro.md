...
In this post I'm going to a make a small intro to the Deep Reinforcement Learning field. It is based on the topics given in the [MIT 6.S094: Deep Reinforcement Learning](https://www.youtube.com/watch?v=MQ6pP65o7OM).

**Reinforcement learning** is a general purpose framework for decision making used as part of the design of artificial devices to interact with the environment. The **agent** is programmed for achieving a **goal** through a set of temporal **actions** that change the internal **state** receiving a measured **reward** that can be positive or negative. The agent is programmed for maximizing the reward.

![RL components]({{ site.url }}/assets/images/2018-02-19-deep-reinforcement-learning-intro/RL.png){: width="150px" style="float: right;margin-right: 7px;margin-top: 7px;"} **Sensors** take **data** from the **environment**, make a **feature extraction** from data for taking the important elements that allow building a **representation hierarchy** of the environment. A set of **machine learning** techniques are applied to convert the features into **knowledge** building a *taxonomy* with the simple and useful information valid for the decision process. With this knowledge, the agent makes the **reasoning** for inferring conclusions about the knowledge and **planning** based on the goals, materialized in the maximization of a *reward function*. From planning an **action** is derived and the **effector** apply the decided actions into the environment.
...
We sort by order of difficulty the type of tasks that we would like to solve in AI. Some of them have been already solved, others are open problems.
...
Machine learning can be classified in two categories: supervised learning & unsupervised learning. 
...
This algorithm used a Montecarlo Tree seach (MCTS) combined with a convolutional neural network to get the intuitions about which positions to expand. This combined strategy was used to balance exploration vs exploitation equilibria.
